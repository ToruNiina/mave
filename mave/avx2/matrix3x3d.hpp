...
struct alignas(32) matrix<double, 3, 3>
{
    ...
    {
        pointer self = this->data();
        _mm256_store_pd(self,   _mm256_setzero_pd());
        _mm256_store_pd(self+4, _mm256_setzero_pd());
        _mm256_store_pd(self+8, _mm256_setzero_pd());
        return;
    }
...
{
    return std::make_tuple(floor(m1), floor(m2));
}
template<>
MAVE_INLINE std::tuple<matrix<double, 3, 3>, matrix<double, 3, 3>,
                       matrix<double, 3, 3>>
...
{
    return std::make_tuple(ceil(m1), ceil(m2));
}
template<>
MAVE_INLINE std::tuple<matrix<double, 3, 3>, matrix<double, 3, 3>,
                       matrix<double, 3, 3>>
