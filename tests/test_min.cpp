...
BOOST_AUTO_TEST_CASE_TEMPLATE(min, T, test_targets)
{
    ...
    for(std::size_t i=0; i<N; ++i)
    {
        ...
        BOOST_TEST(v3[0] == std::min(v1[0], v2[0]), mave::test::tolerance<typename T::value_type>());
        BOOST_TEST(v3[1] == std::min(v1[1], v2[1]), mave::test::tolerance<typename T::value_type>());
        BOOST_TEST(v3[2] == std::min(v1[2], v2[2]), mave::test::tolerance<typename T::value_type>());
    }
}
